...
from recommendation_system import generate_user_item_matrix, generate_prediction_df
...
def parse_args():
    ...
    parser.add_argument('--api_key', default=os.getenv('GOOGLE_MAPS_API_KEY', 'key'), help='Google Maps API key.')
...
def main():
    ...
    try:
        ...
        logging.info("Generating user-item interaction matrix...")
        user_item_matrix = generate_user_item_matrix(final_data)
        logging.info("Predicting ratings...")
        pt_df = user_item_matrix.fillna(0)
        pred_df = generate_prediction_df(pt_df, 10)

        pred_path = os.path.join(args.output_dir, 'predictions.csv')
        pred_df.to_csv(pred_path)
        logging.info(f"Predictions saved at {pred_path}")
